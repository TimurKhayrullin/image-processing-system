...
public:
    ...
    virtual bool load(const std::string &filepath, std::vector<unsigned char> &outPixels, ImageHeader &header) const = 0;
};
...
public:
    ...
    bool load(const std::string &filepath, std::vector<unsigned char> &outPixels, ImageHeader &header) const override;
};
